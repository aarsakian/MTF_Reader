...
func Unmarshal(data []byte, v interface{}) error {
	...
	for i := 0; i < structValPtr.Elem().NumField(); i++ {
		...
		case reflect.String:
			...
			if name == "MagicNumber" {
				if idx+4 > len(data) {
					return errors.New("not enough data for MagicNumber")
				}
		...
		case reflect.Struct:
			...
			if nameType == "MTF_TAPE_ADDRESS" {
				if idx+4 > len(data) {
					return errors.New("not enough data for MTF_TAPE_ADDRESS")
				}
				var mtf_tape_address MTF_TAPE_ADDRESS
				if err := Unmarshal(data[idx:idx+4], &mtf_tape_address); err != nil {
					return err
				}
				...
				if idx+5 > len(data) {
					return errors.New("not enough data for MTF_DATE_TIME")
				}
				var mtf_date_time MTF_DATE_TIME
				if err := Unmarshal(data[idx:idx+5], &mtf_date_time); err != nil {
					return err
				}
		...
		case reflect.Uint8:
			if idx+1 > len(data) {
				return errors.New("not enough data for uint8")
			}
			var temp uint8
		...
		case reflect.Uint16:
			if idx+2 > len(data) {
				return errors.New("not enough data for uint16")
			}
			var temp uint16
		...
		case reflect.Uint32:
			if idx+4 > len(data) {
				return errors.New("not enough data for uint32")
			}
			var temp uint32
		...
		case reflect.Uint64:
			if idx+8 > len(data) {
				return errors.New("not enough data for uint64")
			}
			var temp uint64
		...
		case reflect.Bool:
			if idx+1 > len(data) {
				return errors.New("not enough data for bool")
			}
			field.SetBool(false)
			idx += 1
		case reflect.Array:
			if idx+field.Len() > len(data) {
				return errors.New("not enough data for array field")
			}
			...
			for j := 0; j < field.Len(); j++ {
				arr.Index(j).Set(reflect.ValueOf(data[idx+j]))
			}
